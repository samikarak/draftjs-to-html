...
function getStyleArrayForBlock(block) {
  ...
  forEach(styleConfig, function (styleKey) {
  ...
  if (inlineStyleRanges && inlineStyleRanges.length > 0) {
    inlineStyleRanges.forEach(function (range) {
      ...
      var _loop = function _loop(i) {
        var fulfilled = false;
        forEach(styleConfig, function (styleKey, config) {
          var hasMarkup = !!config.markup;
          if (!hasMarkup) {
            var keyLowerCase = styleKey.toLowerCase();
...
function getStylesAtOffset(inlineStyles, offset) {
  var styles = {};
  forEach(inlineStyles, function (styleKey) {
...
function addInlineStyleMarkup(style, content) {
  var markup = styleConfig[style] && styleConfig[style].markup;
...
function addStylePropertyMarkup(styles, text) {
  var styleStr = '';
  forEach(styles, function (key, val) {
    var hasMarkup = styleConfig[key] && styleConfig[key].markup;
    if (!hasMarkup && styleConfig[key]) {
      if (!styleStr.length) styleStr = 'style="';
      var unit = styleConfig[key].unit;
...
function getInlineStyleSectionMarkup(block, styleSection) {
  ...
  var styleTagSections = getInlineStyleSections(block, keysWithMarkup, styleSection.start, styleSection.end);
...
function getSectionMarkup(block, entityMap, section, customEntityTransform) {
  ...
  var keysWithNoMarkup = [];
  forEach(styleConfig, function (styleKey) {
    if (!styleConfig[styleKey].markup) keysWithNoMarkup.push(styleKey);
  });
  var inlineStyleSections = getInlineStyleSections(block, keysWithNoMarkup, section.start, section.end);
...
function emptyBlockMarkup(markup) {
  var removeTagsMarkup = markup.replace(/<(\/)?\s*(\w+)[^>]*>/g, '').replace(/\s+/g, '');
...
function getBlockInnerMarkup(block, entityMap, hashtagConfig, customEntityTransform) {
  ...
  var markup = blockMarkupArr.join('');
  var addLineBreak = emptyBlockMarkup(markup);
